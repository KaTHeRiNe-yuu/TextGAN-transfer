...
class Model(object):
    ...
    def __init__(self, args, vocab):
        dim_emb = args.dim_emb  # 嵌入维度
        dim_z = args.dim_z  # 隐状态维度
        n_layers = args.n_layers  # 网络层数
        ...
        embedding = tf.get_variable('embedding',
            initializer=vocab.embedding.astype(np.float32))  # 将整数索引转化为输入向量传递给神经网络
        with tf.variable_scope('projection'):
            proj_W = tf.get_variable('W', [dim_z, vocab.size])
            proj_b = tf.get_variable('b', [vocab.size])

        inputs = tf.nn.embedding_lookup(embedding, self.inputs)
        cell = create_cell(dim_z, n_layers, self.dropout)  # 创建RNN神经元 添加dropout层 防止过拟合
...
def get_lm_batches(x, word2id, batch_size):
    pad = word2id['<pad>']
    go = word2id['<go>']  # 起始符号
    eos = word2id['<eos>']  # 结束符号
    unk = word2id['<unk>']

    x = sorted(x, key=lambda i: len(i))  # 将训练集中的句子按照长度排序
...
if __name__ == '__main__':
    ...
    if args.train:
        train = load_sent(args.train)  # 与类别无关
